...
//template <typename T>
//StatusCode CheckoutImpl::createPath(const Path &path, upnsSharedPointer<T> createLeaf, bool exclusive)
//{
//    StatusCode s;
//    Path p = preparePath(path);
//    upnsSharedPointer<Tree> current;
...
//        rootOid->append(toid);
//        assert(!rootMissing || rootMissing && m_checkout->transientoidstoorigin().count(toid) == 0);
//        m_checkout->mutable_transientoidstoorigin()
//                ->insert(::google::protobuf::MapPair< ::std::string, ::std::string>(toid, origin));
//        s = m_serializer->storeCheckoutCommit(m_checkout, m_name);
//        if(!upnsIsOk(s)) return s;
//    }
//    else
...
//    upnsVec<Tree> exculsiveTreePath;
...
//                leafCreated = false;
//                assert(current);
//                if(!exclusive && upnsIsOk(s))
//                {
//                    this->createPath(p.substr(0, idx), upnsSharedPointer<Tree>(nullptr), true);
//                }
//            }
//            else
//            {
...
//                if(!exclusive && upnsIsOk(s))
//                {
//                    this->createPath(p.substr(0, idx), upnsSharedPointer<T>(nullptr), true);
//                }
//            }
//            if(upnsIsOk(s))
//            {
...
//                ::std::string nam = seg;
//                assert(current);
//                current->mutable_refs()
//                        ->insert( ::google::protobuf::MapPair< ::std::string, ::upns::ObjectReference>( nam, oref));
//                s = m_serializer->storeTree(current);
//                log_info("dbg: Added obj " + p
//                         + "name: " + seg
//                         + "id: " + nextOid);
...
//            if(!upnsIsOk(s))
//            {
//                log_error("Could not create Leaf for path \"" + p + "\", segment: \"" + seg + "\"" );
//                return false;
//            }
//            if(leafCreated) return false; // we are done
//        }
//        current = m_serializer->getTree(oid);
//        return true; // continue thru path
//    },
