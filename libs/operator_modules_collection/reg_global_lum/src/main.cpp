...
mapit::StatusCode operate_reg_global_lum(mapit::OperationEnvironment* env)
{
    /** structure:
     * {
    ...
    mapit::RegGlobalLUM reg_global(env, status);
    if ( ! mapitIsOk( status ) ) {
        return status;
    }
...
MAPIT_MODULE(  OPERATOR_NAME
             , "execute global scanmatching on pointclouds using LUM ()"
             , "fhac"
             , OPERATOR_VERSION
             , "any"
             , true
             , &operate_reg_global_lum)
