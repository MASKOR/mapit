...
Layer::LayerType PointcloudLayerdata::layerType() const
{
    return Layer::LayerType::POINTCLOUD2;
}

int _PclPcdRead (std::istream &fs, pcl::PCLPointCloud2 &cloud);

LayerDataHandle PointcloudLayerdata::getData(upnsReal x1, upnsReal y1, upnsReal z1, upnsReal x2, upnsReal y2, upnsReal z2, bool clipMode, int lod)
{
    if(m_pointcloud.height == 0)
    {
        upnsIStream &in = m_streamProvider->startRead();
        _PclPcdRead(in, m_pointcloud);
        m_streamProvider->endRead(in);
    }
    return static_cast<LayerDataHandle>(&m_pointcloud);
}

int PointcloudLayerdata::setData(upnsReal x1, upnsReal y1, upnsReal z1, upnsReal x2, upnsReal y2, upnsReal z2, LayerDataHandle &data, int lod)
{
    upnsOStream &out = m_streamProvider->startWrite();
    out << ((pcl::PCLPointCloud2*)data);
