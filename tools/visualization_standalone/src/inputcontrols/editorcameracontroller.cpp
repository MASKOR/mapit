...
void EditorCameraController::adjustCamera(const QVector3D &translateVec)
{
    ...
    QVector3D intersection = EditorCameraController::findIntersection(newPosition,
                                                           m_camera->viewVector().normalized(),
                                                           0, EditorCameraController::cameraNormal(m_camera),
                                                           t);
    if (t > 0.0f)
...
{
    handleWheelScroll(event->angleDelta().y(), event->pos());
}

void EditorCameraController::handleWheelScroll(int delta, const QPoint &mousePosition)
{
    if (m_camera) {
        // Find intersection of cursor and camera far plane.
        QVector3D planeNormal = EditorCameraController::cameraNormal(m_camera);
        ...
        QVector3D ray = EditorCameraController::unprojectRay(m_camera->viewMatrix(),
                                                  m_camera->projectionMatrix(),
                                                  m_viewportSize.width(), m_viewportSize.height(),
                                                  mousePosition);
        float t = 0.0f;
        QVector3D intersection = EditorCameraController::findIntersection(m_camera->position(), ray,
        ...
        QMatrix4x4 viewMatrix = m_camera->viewMatrix().inverted();
        QVector3D translateVec(0.0f, 0.0f, delta * m_wheelSpeed);
...
QVector3D EditorCameraController::cameraNormal(Qt3DRender::QCamera *camera)
{
...
QVector3D EditorCameraController::findIntersection(const QVector3D &rayOrigin, const QVector3D &ray,
...
QVector3D EditorCameraController::unprojectRay(const QMatrix4x4 &viewMatrix,
...
QSize EditorCameraController::viewportSize() const
{
    return m_viewportSize;
}

void EditorCameraController::setViewportSize(QSize viewportSize)
{
